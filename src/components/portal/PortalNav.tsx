...
export default async function PortalNav() {
  ...
  return (
    <header className="portal-rise" style={{ position: "sticky", top: 0, zIndex: 80, height: 70, background: "var(--portal-paper)", display: "flex", alignItems: "center", gap: 28, padding: "0 clamp(16px,4vw,48px)", borderBottom: "1px solid var(--portal-line)", animationDuration: "0.4s" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
        {navLinks.map((n) => (
          <Link key={n.href} href={n.href} className="portal-nav-link" style={{ padding: "9px 14px", fontSize: 14, fontWeight: 700, letterSpacing: "-.01em", color: "var(--portal-ink)" }}>
            {n.label}
          </Link>
        ))}
      </nav>
      ...
      {user ? (
        <Link href="/account" className="portal-choice" style={{ display: "flex", alignItems: "center", gap: 9, background: "#eae9e9", padding: "7px 14px 7px 8px", color: "var(--portal-ink)" }}>
